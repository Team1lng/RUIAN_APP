...
lv_img_cache_entry_t *_lv_img_cache_open(const void *src, lv_color_t color)
{
	...
	for (i = 0; i < entry_cnt; i++)
	{
		if (color.full == cache[i].dec_dsc.color.full && lv_img_cache_match(src, cache[i].dec_dsc.src))
		{
...
void lv_img_cache_invalidate_src_all(void)
{
#if LV_IMG_CACHE_DEF_SIZE
	lv_img_cache_entry_t *cache = LV_GC_ROOT(_lv_img_cache_array);
...
static bool lv_img_cache_match(const void *src1, const void *src2)
{
	...
	rom_bin_info *png1_src_info = (rom_bin_info *)src1;
	rom_bin_info *png2_src_info = (rom_bin_info *)src2;
	if (png1_src_info->offset != png2_src_info->offset)
	{
		return false;
	}
	return true;
    ...
    if(lv_img_src_get_type(src2) != LV_IMG_SRC_FILE)
        return false;
    return true;//strcmp(src1, src2) == 0;
#endif
}
#endif
