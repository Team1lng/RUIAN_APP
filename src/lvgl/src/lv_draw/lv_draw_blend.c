...
#elif LV_USE_GPU
            if(disp->driver.gpu_blend_cb && (lv_area_get_width(draw_area) > GPU_SIZE_LIMIT) &&(lv_area_get_height(draw_area) > GPU_SIZE_LIMIT))
			{
				disp->driver.gpu_blend_cb(&disp->driver, disp_buf, &color, draw_area, draw_area, opa,true);
#if 0
            ...
            for(y = 0; y < draw_area_h; y++) 
			{
                for(x = 0; x < draw_area_w; x++) 
				{
                    if(last_dest_color.full != disp_buf_first[x].full) 
					{
						...
						{
                            lv_color_mix_with_alpha(disp_buf_first[x], disp_buf_first[x].ch.alpha, color, opa, &last_res_color, &last_res_color.ch.alpha);
                        }
                        else
#endif
                        {
                                             ...
                                             const lv_opa_t * mask, lv_draw_mask_res_t mask_res)
{
    ...
    if(mask_res == LV_DRAW_MASK_RES_FULL_COVER)
	{
        ...
        if(disp->driver.gpu_blend_cb &&  (lv_area_get_width(draw_area) > GPU_SIZE_LIMIT) &&(lv_area_get_height(draw_area) > GPU_SIZE_LIMIT))
		{
			 disp->driver.gpu_blend_cb(&disp->driver, disp_buf, map_buf, map_area,draw_area, opa,false);
#if 0
            for(y = draw_area->y1; y <= draw_area->y2; y++)
			{
        ...
        else
		{
            ...
            for(y = 0; y < draw_area_h; y++) 
			{
                for(x = 0; x < draw_area_w; x++) 
				{
                    ...
                    if(disp->driver.screen_transp) 
					{
                        lv_color_mix_with_alpha(disp_buf_first[x], disp_buf_first[x].ch.alpha, map_buf_first[x], opa, &disp_buf_first[x],&disp_buf_first[x].ch.alpha);
                    }
                    else
#endif
                    {
            ...
            for(y = 0; y < draw_area_h; y++)
			{
                for(x = 0; x < draw_area_w; x++)
				{
                    if(mask[x]) 
					{
                        ...
                        if(disp->driver.screen_transp)
						{
                            lv_color_mix_with_alpha(disp_buf_first[x], disp_buf_first[x].ch.alpha, map_buf_first[x], opa_tmp, &disp_buf_first[x], &disp_buf_first[x].ch.alpha);
                        }
                        else
#endif
                        {
