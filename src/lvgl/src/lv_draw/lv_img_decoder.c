...
lv_res_t lv_img_decoder_open(lv_img_decoder_dsc_t *dsc, const void *src, lv_color_t color)
{
	...
	if (dsc->src_type == LV_IMG_SRC_FILE)
	{
		...
		dsc->src = lv_mem_alloc(sizeof(rom_bin_info)); // lv_mem_alloc(strlen(src_img) + 1);
		...
		memcpy((char *)dsc->src, src, sizeof(rom_bin_info)); // strcpy(new_str, src_img);
#if 0
	...
	_LV_LL_READ(LV_GC_ROOT(_lv_img_defoder_ll), d)
	{
		...
		if (d->info_cb == NULL || d->open_cb == NULL)
			continue;
