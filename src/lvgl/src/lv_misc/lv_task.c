...
static bool lv_task_exec(lv_task_t *task)
{
    ...
    if (lv_task_time_remaining(task) == 0)
    {
        ...
        { /*The task might be deleted by itself as well*/
...
** 返回参数说明：
***/
lv_task_t *lv_layout_task_create(lv_task_cb_t task_xcb, uint32_t period, lv_task_prio_t prio, void *user_data)
{
    lv_task_t *task_t = lv_task_create(task_xcb, period, prio >= LV_REFR_TASK_PRIO ? (LV_REFR_TASK_PRIO - 1) : prio, user_data);
    task_t->clean_lock = true;
    return task_t;
}
