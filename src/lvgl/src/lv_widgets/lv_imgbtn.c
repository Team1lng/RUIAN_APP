    ...
    /*Draw the object*/
        ...
        if(!ext->tiled) {
			...
			//modiy by leo
				...
				lv_coord_t obj_h = lv_obj_get_height(imgbtn);
		        coords_part.x1 = imgbtn->coords.x1 + (obj_w - header.w) / 2;
		        coords_part.y1 = imgbtn->coords.y1 + (obj_h - header.h) / 2;
...
static void refr_img(lv_obj_t * imgbtn)
{
	...
	/* modiy by leo*/
        ...
        if(ext->tiled)
		{
			lv_obj_set_height(imgbtn, header.h); /*Keep the sure defined width*/
        }
        else
		{
