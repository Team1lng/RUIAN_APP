...
#ifdef LV_USE_PATTERN
    ext->pattern_same   = 1;
	ext->pattern_p      = NULL;
	ext->pattern_align  = LV_LABEL_ALIGN_LEFT;
#endif
    ...
    while(1) {
        ...
        if(btn_cnt != 0) {
            ...
            for(i = 0; i < btn_cnt; i++) {
                ...
                {
                    lv_area_set(&ext->button_areas[btn_i], ext->button_areas[btn_i - 1].x2, act_y, act_x + act_unit_w,act_y + btn_h);
                }
                else
				{
 ...
 */
void lv_btnmatrix_set_align(lv_obj_t * btnm, lv_label_align_t align,lv_coord_t x_ofs, lv_coord_t y_ofs)
{
    LV_ASSERT_OBJ(btnm, LV_OBJX_NAME);

    lv_btnmatrix_ext_t * ext = lv_obj_get_ext_attr(btnm);
    if((ext->align == align)&&(ext->x_ofs == x_ofs)&&(ext->y_ofs == y_ofs)) return;
...
#ifdef LV_USE_PATTERN
void lv_btnmatrix_set_pattern_image(lv_obj_t* btnm,uint16_t btn_id,rom_bin_info* info)
{
	lv_btnmatrix_ext_t * ext = lv_obj_get_ext_attr(btnm);
	if(btn_id > ext->btn_cnt)
	{
		 LV_LOG_WARN("button matrix btn id out");
		return ;	
	}

	ext->pattern_p[btn_id] = info;
	ext->pattern_same = 0;
}

void lv_btnmatrix_set_pattern_align(lv_obj_t* btnm,uint8_t align)
{
	lv_btnmatrix_ext_t * ext = lv_obj_get_ext_attr(btnm);
	ext->pattern_align = align;
}
...
static lv_design_res_t lv_btnmatrix_design(lv_obj_t * btnm, const lv_area_t * clip_area, lv_design_mode_t mode)
{
    ...
    /*Draw the object*/
        ...
        for(btn_i = 0; btn_i < ext->btn_cnt; btn_i++, txt_i++) {
...
#ifdef LV_USE_PATTERN
            if(ext->pattern_same == 0)
            {
                if(ext->pattern_p[btn_i] != NULL)
                {
                    draw_rect_dsc_act->pattern_image = (void*)ext->pattern_p[btn_i];
                    if(ext->pattern_align == LV_LABEL_ALIGN_CENTER)
                    {
                        draw_rect_dsc_act->pattern_center = 1;
                    }
                    else
                    {
                        draw_rect_dsc_act->pattern_center = 0;
                    }
                }
                else
                {
                    draw_rect_dsc_act->pattern_image = NULL;
                }
            }            
#endif			
            ...
            lv_point_t txt_size;
            _lv_txt_get_size(&txt_size, txt, font, letter_space,line_space, lv_area_get_width(&area_btnm), txt_flag);

			area_tmp.x1 += ext->x_ofs;
            ...
            area_tmp.y1 += (btn_h - txt_size.y) / 2 + ext->y_ofs;

			area_tmp.x2 += ext->x_ofs;
        
            area_tmp.y2 = area_tmp.y1 + txt_size.y + ext->y_ofs;
...
#ifdef LV_USE_PATTERN
	ext->pattern_p = lv_mem_alloc(sizeof(rom_bin_info*) * btn_cnt);
	LV_ASSERT_MEM(ext->pattern_p);
    _lv_memset_00(ext->pattern_p, sizeof(rom_bin_info*) * btn_cnt);
#endif	

    if(ext->button_areas == NULL || ext->ctrl_bits == NULL || ext->pattern_p == NULL) btn_cnt = 0;
