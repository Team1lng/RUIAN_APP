...
void lv_label_set_text(lv_obj_t * label, const char * text)
{
    ...
    lv_obj_invalidate(label);//无效并且重绘 清除label标签

    lv_label_ext_t * ext = lv_obj_get_ext_attr(label);//获取扩展指针
...
static lv_design_res_t lv_label_design(lv_obj_t * label, const lv_area_t * clip_area, lv_design_mode_t mode)
{
    ...
    else if(mode == LV_DESIGN_DRAW_MAIN) 
	{
        ...
        if(ext->long_mode == LV_LABEL_LONG_SROLL_CIRC) {
            ...
            if(size.x > lv_area_get_width(&txt_coords))
			{
                label_draw_dsc.ofs_x = ext->offset.x + size.x + lv_font_get_glyph_width(label_draw_dsc.font, ' ', ' ') * LV_LABEL_WAIT_CHAR_COUNT;
                label_draw_dsc.ofs_y = ext->offset.y;
