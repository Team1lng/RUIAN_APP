...
void lv_textarea_set_pwd_mode(lv_obj_t * ta, bool en)
{
    ...
    /*Pwd mode is now enabled密码模式启用*/
    if(en != false) {
        char * txt   = lv_label_get_text(ext->label);//获取当前的文本内容
...
static void pwd_char_hider(lv_obj_t * ta)
{
    lv_textarea_ext_t * ext = lv_obj_get_ext_attr(ta);//获取扩展区域
    if(ext->pwd_mode != 0) {
        ...
        const char * bullet;
        if(has_bullet) bullet = "ERROR";//LV_SYMBOL_BULLET;
        else bullet = "*";
		LV_LOG_ERROR("draw_header:	There is a problem here that needs to be fixed \n");
#else 
		const char * bullet = "*";
#endif
