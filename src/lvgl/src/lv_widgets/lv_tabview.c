...
lv_obj_t * lv_tabview_create(lv_obj_t * par, const lv_obj_t * copy)
{
...
#if LV_USE_ANIMATION
    ext->anim_time = 0;//LV_TABVIEW_DEF_ANIM_TIME;
#endif
    ...
    if(copy == NULL) {
        ...
        lv_page_set_scrollable_fit2(ext->content,LV_FIT_PARENT,LV_FIT_TIGHT);//LV_FIT_TIGHT, LV_FIT_PARENT);
        lv_page_set_scrl_layout(ext->content, LV_LAYOUT_COLUMN_LEFT);//LV_LAYOUT_ROW_TOP);
...
void lv_tabview_set_tab_act(lv_obj_t * tabview, uint16_t id, lv_anim_enable_t anim)
{
    ...
    switch(ext->btns_pos) {
        ...
        case LV_TABVIEW_TAB_POS_LEFT:
        case LV_TABVIEW_TAB_POS_RIGHT:
            cont_x = -((lv_obj_get_height(tabview) /*- lv_obj_get_width(ext->btns)*/) * id + scrl_inner * id + scrl_left);
            break;
    }

    if(anim == LV_ANIM_OFF || lv_tabview_get_anim_time(tabview) == 0) {
        lv_obj_set_y(lv_page_get_scrollable(ext->content), cont_x);
    }
#if LV_USE_ANIMATION
    else {
        lv_anim_t a;
        ...
        lv_anim_set_values(&a, lv_obj_get_y(lv_page_get_scrollable(ext->content)), cont_x);
...
static lv_res_t lv_tabview_signal(lv_obj_t * tabview, lv_signal_t sign, void * param)
{
    ...
    if(sign == LV_SIGNAL_CLEANUP) {
		...
		for(i = 0 ; i < (ext->tab_cnt * 2 ) ;i++)
		{
			if(ext->tab_name_ptr[i][0] != '\0' && ext->tab_name_ptr[i][0] != '\n')
			{
...
static lv_res_t tabview_scrl_signal(lv_obj_t * tabview_scrl, lv_signal_t sign, void * param)
{
    ...
    if(sign == LV_SIGNAL_DRAG_THROW_BEGIN) {
        ...
        lv_coord_t page_x1  = tab_page->coords.y1 - tabview->coords.y1 + x_predict;
        lv_coord_t page_x2  = page_x1 + lv_obj_get_height(tabview);
        lv_coord_t threshold = lv_obj_get_height(tabview) / 2;
