...
static lv_design_res_t lv_roller_design(lv_obj_t * roller, const lv_area_t * clip_area, lv_design_mode_t mode)
{
    ...
    /*Post draw when the children are drawn*/
		...
		//	unsigned long long start = get_sys_ms();
           ...
           // _lv_txt_get_size(&res_p, lv_label_get_text(label), label_dsc.font, label_dsc.letter_space, label_dsc.line_space, lv_obj_get_width(roller), LV_TXT_FLAG_EXPAND);
			printf("%s %d %llu \n",__func__,__LINE__,get_sys_ms() - start);
            ...
            label_sel_area.y1 = label->coords.y1;//label_sel_y;
            label_sel_area.x2 = label->coords.x2;
            label_sel_area.y2 = label->coords.y2;//label_sel_area.y1 + res_p.y;
