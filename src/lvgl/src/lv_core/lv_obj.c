...
lv_font_t* LV_FONT_SIZE_20 = NULL;
lv_font_t* LV_FONT_SIZE_24 = NULL;
lv_font_t* LV_FONT_SIZE_28 = NULL;
lv_font_t* LV_FONT_SIZE_31 = NULL;
lv_font_t* LV_FONT_SIZE_36 = NULL;
lv_font_t* LV_FONT_SIZE_42 = NULL;
 ...
 *   GLOBAL FUNCTIONS
 **********************/
static unsigned int rom_bin_size = 0;
const unsigned char* get_rom_bin_base(void)
{
	static unsigned char* rom_bin_base = NULL;
	if(rom_bin_base == NULL)
	{
		struct stat statbuf;
    	int ret = stat(ROM_BIN_PATH,&statbuf);//调用stat函数
		if(ret != 0)
		{
			LV_LOG_ERROR("get %s fail\n",ROM_BIN_PATH);
			return NULL;
		}

		int fd = open(ROM_BIN_PATH,O_RDONLY|O_CREAT);
		if(fd < 0)
		{
			LV_LOG_ERROR("rom bin base addres fail:%s\n",ROM_BIN_PATH);
			return NULL;

		}
		rom_bin_size = statbuf.st_size;
   	 	rom_bin_base = (unsigned char*)malloc(rom_bin_size);

		read(fd,rom_bin_base,statbuf.st_size);
	}
	return rom_bin_base;
}
unsigned int get_rom_bin_size(void)
{
	return rom_bin_size;
}
...
void lv_init(void)
{
	...
	extern void lv_png_init(const unsigned char*,unsigned int);
	lv_png_init(get_rom_bin_base(),get_rom_bin_size());



	extern bool lv_freetype_init(uint16_t max_faces, uint16_t max_sizes, uint32_t max_bytes);
	lv_freetype_init(0,0,1024*512);//1024*1000

	extern bool lv_ft_font_init(lv_font_t ** info,int size);
	lv_ft_font_init(&LV_FONT_SIZE_20,20);

	lv_ft_font_init(&LV_FONT_SIZE_24,24);

	lv_ft_font_init(&LV_FONT_SIZE_28,28);
	lv_ft_font_init(&LV_FONT_SIZE_31,31);
	lv_ft_font_init(&LV_FONT_SIZE_36,36);
    lv_ft_font_init(&LV_FONT_SIZE_42,42);

    lv_theme_t * th = LV_THEME_DEFAULT_INIT(LV_THEME_DEFAULT_COLOR_PRIMARY,
...
#if 0
	extern void lv_png_init(const unsigned char*);
	lv_png_init(get_rom_bin_base());
	extern bool lv_freetype_init(unsigned int, const unsigned char* ,unsigned int);
	lv_freetype_init(64,get_rom_bin_base() + ROM_RES_FONT_SAT_LEO_TTF,ROM_RES_FONT_SAT_LEO_TTF_SIZE);
#endif
...
#if LV_USER_DEFINE_OBJ_ID
	new_obj->obj_id = 0xFFFFFF;
#endif
...
#if LV_USER_DEFINE_OBJ_ID
lv_obj_t * lv_obj_get_child_form_id(const lv_obj_t * obj,unsigned int id)
{
    LV_ASSERT_OBJ(obj, LV_OBJX_NAME);

    lv_obj_t * result = _lv_ll_get_head(&obj->child_ll);
	while(result != NULL)
	{
	//	printf("child id:%d %d\n",result->obj_id,id);
		if(result->obj_id == id)
		...
		result = _lv_ll_get_next(&obj->child_ll, result);
	}
    return NULL;
}

void lv_obj_set_id(lv_obj_t* obj,unsigned int id)
{
	 LV_ASSERT_OBJ(obj, LV_OBJX_NAME);
	 ...
	 if(parent != NULL)
	 {
		if(lv_obj_get_child_form_id(parent,id) != NULL)
		{
            printf("duplicate id:%d\n",id);
			LV_LOG_ERROR("lv_obj_set_id: Duplicate ID setting");
			return ;
		}
		obj->obj_id = id;
	 }
	 ...
	 {
		 LV_LOG_ERROR("lv_obj_set_id: no parent \n");
	 }
}

unsigned int lv_obj_get_id(lv_obj_t *obj)
{
	LV_ASSERT_OBJ(obj, LV_OBJX_NAME);
