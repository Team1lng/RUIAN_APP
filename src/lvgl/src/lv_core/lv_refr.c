...
void _lv_disp_refr_task(lv_task_t *task)
{
	...
	// time_ms_printf(__func__,__LINE__);
...
#if LV_USE_GPU_STM32_DMA2D
						lv_gpu_stm32_dma2d_copy((lv_color_t *)(buf_act + start_offs), disp_refr->driver.hor_res, (lv_color_t *)(buf_ina + start_offs), disp_refr->driver.hor_res,
...
static void lv_refr_areas(void)
{
	...
	for (i = 0; i < disp_refr->inv_p; i++)
	{
		...
		{
			if (i == last_i)
				disp_refr->driver.buffer->last_area = 1;
...
static void lv_refr_obj_and_children(lv_obj_t *top_p, const lv_area_t *mask_p)
{
	...
	if (top_p == NULL)
		top_p = lv_disp_get_scr_act(disp_refr);
	if (top_p == NULL)
