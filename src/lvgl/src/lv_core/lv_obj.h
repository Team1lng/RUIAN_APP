...
#define LV_ASSERT_OBJ(obj, obj_type)
	...
	typedef struct
	{
		...
		const unsigned char type; /***** default:0:PNG,1:RAW*****/
		unsigned long offset;
		unsigned long size;
	} rom_bin_info;

#define rom_bin_info_get(x)         \
	{                           \
		'L', 0, x, x##_SIZE \
	}

#define rom_bin_raw_get()    \
	{                    \
		.name = 'L', \
		.type = 1,   \
		.offset = 0, \
...
#define rom_bin_raw_init(r, a, w, h)   \
	r.offset = (unsigned long)(a); \
	r.size = (h << 12) | w;

	extern lv_font_t *LV_FONT_SIZE_20;
	extern lv_font_t *LV_FONT_SIZE_24;
	extern lv_font_t *LV_FONT_SIZE_28;
	extern lv_font_t *LV_FONT_SIZE_31;
	extern lv_font_t *LV_FONT_SIZE_36;
	extern lv_font_t *LV_FONT_SIZE_42;

#define FONT_SIZE(x) (LV_FONT_SIZE_##x)
