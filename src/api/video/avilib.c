...
#define malloc(size) ak_mem_alloc(MODULE_ID_APP, size)
#define free(p) ak_mem_free(p)
/*******************************************************************
 *                                                                 *
...
void AVI_set_audio(avi_t *AVI, int channels, long rate, int bits, int format, long mp3rate)
{
   ...
   if(AVI->anum > AVI_MAX_TRACKS) {
     printf( "error - only %d audio tracks supported\n", AVI_MAX_TRACKS);
     exit(1);
   }
...
//	printf("ch: %d==rate:%d======bits:%d===format:%d==========mp3:%d============>>>\n\r",channels,rate,bits,format,mp3rate);
   //avi_update_header(AVI);
}
...
#define AVIF_COPYRIGHTED        0x00020000
   ...
   /* Output the header, truncate the file to the number of bytes
	...
	if(lseek(AVI->fdes,0,SEEK_SET)<0 || avi_write(AVI->fdes,(char *)AVI_header,HEADERBYTES)!=HEADERBYTES || lseek(AVI->fdes,AVI->pos,SEEK_SET)<0)
	{
...
long AVI_cur_frame_size(avi_t *AVI)
{
   if(AVI->mode==AVI_MODE_WRITE) { AVI_errno = AVI_ERR_NOT_PERM; return -1; }
   if(!AVI->video_index)         { AVI_errno = AVI_ERR_NO_IDX;   return -1; }

   return(AVI->video_index[AVI->video_pos].len);
}
