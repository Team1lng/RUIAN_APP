...
#define SETTING_PASSWORD_LABEL_APPLE_BTN 0X07
...
static char floor[2] = {0};
static void setting_password_keyboard_enter_password_btn_up(lv_obj_t *obj, lv_event_t ev)
{
    lv_obj_t* label = lv_obj_get_child_form_id(lv_scr_act(), SETTING_PASSWORD_LABEL_APPLE_BTN);
	if (ev != LV_EVENT_RELEASED)
	{
		return;
	}
	const char *txt = lv_btnmatrix_get_active_btn_text(obj);
    if (txt[0] >= '0' && txt[0] <= '9' && len < 2)
    {
        printf("=================%s\n", txt);
        floor[len] = txt[0];
        len++;
        lv_label_set_text(label, floor);
    }
    else if (txt[0] == '#')
    {
        printf("=================floor id:%s\n", floor);
        user_data_get()->floor[0] = floor[0];
        memcpy(user_data_get()->floor, floor, 2);
        user_data_save();
        printf("=================user floor id:%s\n", user_data_get()->floor);
    }
    else if (txt[0] == ' ')
    {
        if (len == 1)
...
static void password_text_display(void)
{
    lv_obj_t* label = lv_label_create(lv_scr_act(), NULL);
    lv_obj_set_size(label, 30, 60);
    lv_obj_set_pos(label, 600, 250);
    lv_obj_set_style_local_text_font(label, LV_LABEL_PART_MAIN, LV_STATE_DEFAULT, LV_FONT_SIZE_42);
    lv_label_set_text(label, user_data_get()->floor);
    lv_obj_set_id(label, SETTING_PASSWORD_LABEL_APPLE_BTN);
}



static void LAYOUT_ENETER_FUNC(floor_number_select)
{
    len = 0;
    memset(floor, 0, 2);
