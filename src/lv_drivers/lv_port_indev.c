...
static bool touchpad_read(lv_indev_drv_t * drv, lv_indev_data_t * data)
{
		...
		else if(in.type == EV_KEY) 
		{
            ...
            if(in.code == BTN_MOUSE || in.code == BTN_TOUCH) 
			{
                if(in.value == 0)
                    ...
                    evdev_button = LV_INDEV_STATE_PR;
                    //printf("[硬件事件] 触摸按下:code=%d, value=%d\n", in.code, in.value);
            } 
	...
	data->point.y = map(evdev_root_y, EVDEV_VER_MIN, EVDEV_VER_MAX, 0, lv_disp_get_ver_res(drv->disp));
        ...
        #ifdef _PLATFORM_8inch
        data->point.x = evdev_root_x;
        data->point.y = evdev_root_y;
        #else
        data->point.x = LV_HOR_RES_MAX - (evdev_root_x * 1024 /1280);
        data->point.y = LV_VER_RES_MAX - (evdev_root_y * 600 / 800);
        #endif
    #else
    data->point.x = evdev_root_x;
    data->point.y = evdev_root_y;
